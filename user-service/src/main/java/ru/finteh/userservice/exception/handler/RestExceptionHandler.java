...
import ru.finteh.userservice.exception.BadRequestException;
import ru.finteh.userservice.exception.InternalServerError;
import ru.finteh.userservice.exception.NotFoundException;
import ru.finteh.userservice.exception.RequestTimeOutException;
import ru.finteh.userservice.exception.UnauthorizedUserException;
...
public class RestExceptionHandler {
    ...
    /**
     * Обрабатывает исключение RequestTimeOut и возвращает HTTP-ответ с кодом 408 REQUEST_TIMEOUT.
     *
     * @param runTimeEx исключение RequestTimeOut
     * @return ответ клиенту с сообщением об ошибке и кодом состояния 408
     */
    @ExceptionHandler(RequestTimeOutException.class)
    @ResponseStatus(HttpStatus.REQUEST_TIMEOUT)
    public ErrorResponseDto handleRequestTimeOutException(RuntimeException runTimeEx) {
        return new ErrorResponseDto(runTimeEx.getMessage());
    }
